...
# ===========================================
# TRANSLATION SERVICE - MODE AGGRESIF
# ===========================================
class TranslationService:
    ...
    def translate_to_indonesian(self, text: str):
        """Terjemahkan teks ke Bahasa Indonesia dengan MODE AGGRESIF"""
        if not self.available or not text or text == "Tidak tersedia":
            return text

        # Cek cache dulu (pakai stable hash)
        ...
        try:
            ...
            prompt = f"""
            ANDA HARUS TERJEMAHKAN TEKS INI KE BAHASA INDONESIA 100%.
            JANGAN ADA SATU KATA BAHASA INGGRIS PUN DI HASIL AKHIR.

            TEKS ASLI: "{text}"

            PERATURAN KETAT:
            1. HASIL AKHIR HARUS 100% BAHASA INDONESIA
            2. PERTAHANKAN: angka (500 mg, 10 ml), satuan, nama obat (paracetamol, ibuprofen)
            3. TERJEMAHKAN SEMUA kata kerja, kata sifat, dan penjelasan
            4. Gunakan istilah medis yang umum di Indonesia
            5. JANGAN tambahkan informasi baru
            6. Jika ada istilah teknis, jelaskan dengan bahasa sederhana
            7. Format menjadi kalimat yang mudah dipahami pasien Indonesia
            ...
            "Consult your doctor before use" → "Konsultasikan dengan dokter sebelum menggunakan"
            ...
            # Validasi: pastikan tidak ada kata kunci Inggris yang tidak perlu
            english_keywords = [' the ', ' and ', ' for ', ' with ', ' use ', ' take ', ' dosage ', ' tablet ', ' capsule ']
            has_english = any(keyword in translated.lower() for keyword in english_keywords)

            if has_english:
                # Coba terjemahkan ulang dengan prompt berbeda
                translated = self._translate_medical_simple(text)

            # Validasi akhir: jika terjemahan terlalu pendek, gunakan fallback
            if len(translated) < max(30, len(text) * 0.3):  # Terlalu singkat
    ...
    def _is_definitely_indonesian(self, text: str) -> bool:
        ...
        indonesian_keywords = [
            ...
            'mg', 'ml', 'tablet', 'kapsul', 'dokter', 'apoteker', 'pasien',
            'konsultasikan', 'sebelum', 'setelah', 'makan', 'efek', 'samping',
            'dosis', 'takaran', 'aturan', 'pakai', 'gunanya', 'manfaat',
            'bahaya', 'larangan', 'hindari', 'interaksi', 'bereaksi'
        ]

        # Kata kunci Bahasa Inggris yang tidak boleh ada (kecuali dalam konteks teknis)
        english_keywords = ['the', 'and', 'for', 'with', 'that', 'this', 'are', 'you', 'should', 'may']
        ...
        eng_count = sum(1 for word in words if word in english_keywords)

        # Jika lebih banyak kata Indonesia dan sedikit/tidak ada kata Inggris
        ratio = indo_count / len(words) if len(words) > 0 else 0
        return ratio > 0.2 or (indo_count > 3 and eng_count == 0)

    def _translate_medical_simple(self, text: str):
        """Terjemahan sederhana untuk teks medis"""
        try:
            model = genai.GenerativeModel('gemini-2.0-flash')

            prompt = f"""
            Terjemahkan teks medis berikut ke Bahasa Indonesia:

            "{text}"

            Hasil harus:
            - 100% Bahasa Indonesia
            - Pertahankan angka dan satuan (mg, ml, tablet, dll)
            - Gunakan bahasa sederhana yang mudah dipahami pasien
    ...
    def _translate_fallback(self, text: str):
        """Fallback translation jika semua gagal

        Perbaikan:
        - Lakukan replacement hanya pada whole-word (word boundaries)
        - Hindari mengganti preposisi pendek/generik yang menyebabkan kerusakan kata
        - Urutkan replacement berdasarkan panjang kata descending
        """
        # Simple string replacement untuk istilah umum (HANYA whole-word)
        replacements = {
            # Header FDA
        ...
        # Urutkan key berdasarkan panjang descending agar kata lebih panjang diproses dulu
        for eng, indo in sorted(replacements.items(), key=lambda x: -len(x[0])):
            if not eng.strip():
                continue
            # Hanya replace whole-word (word boundary) untuk menghindari perubahan di tengah kata
            pattern = re.compile(r'\b' + re.escape(eng) + r'\b', re.IGNORECASE)
            result = pattern.sub(indo, result)

        # Bersihkan spasi ganda dan spasi sebelum tanda baca
        result = re.sub(r'\s+', ' ', result).strip()
        result = re.sub(r'\s+([,\.;:])', r'\1', result)

        # Jika hasil masih tampak berantakan (contoh: banyak huruf tidak alfabet), kembalikan versi minimal:
        # potong panjang ekstrem atau fallback ke versi asli yang disederhanakan
        if len(re.findall(r'[a-zA-Z]', result)) < 3 and len(result) < 30:
            # kembalikan teks asli tapi coba replace header besar saja (kasus ekstrim)
            header_replacements = {
                'INDICATIONS AND USAGE': 'INDIKASI DAN PENGGUNAAN',
                'DOSAGE AND ADMINISTRATION': 'DOSIS DAN CARA PEMAKAIAN',
                'ADVERSE REACTIONS': 'EFEK SAMPING',
            ...
            for eng, indo in header_replacements.items():
                result = re.sub(re.escape(eng), indo, text, flags=re.IGNORECASE)
            result = re.sub(r'\s+', ' ', result).strip()
...
class SimpleRAGPharmaAssistant:
    ...
    def _contains_too_much_english(self, text: str) -> bool:
        ...
        return eng_count > 2 and indo_count < 3
...
**PERINGATAN PENTING:** Informasi ini berasal dari database FDA Amerika Serikat dan telah diterjemahkan. Informasi ini hanya untuk tujuan edukasi. **SELALU KONSULTASIKAN DENGAN DOKTER ATAU APOTEKER ANDA SEBELUM MENGGUNAKAN OBAT APAPUN.**
        ...
        try:
            ...
            Pertanyaan: "Apa dosis paracetamol untuk dewasa?"
            Jawaban: "Berdasarkan data resmi dari FDA Amerika Serikat:
            - Dosis paracetamol untuk dewasa biasanya 500-1000 mg setiap 4-6 jam
            - Dosis maksimal harian: 4000 mg
            - Jangan melebihi dosis yang dianjurkan
            ⚠️ KONSULTASIKAN DENGAN DOKTER ATAU APOTEKER SEBELUM MENGGUNAKAN PARACETAMOL."
            ...
            # Daftar kata Inggris yang TIDAK BOLEH ada (kecuali dalam konteks teknis)
            forbidden_english = [
...
def main():
    ...
    # ===========================================
    # HALAMAN CHATBOT
    # (konten tidak diubah selain penggunaan kelas yang diperbarui)
    # ===========================================
    ...
    # (tidak diubah selain memanfaatkan perbaikan di atas)
    # ===========================================
